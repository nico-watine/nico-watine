---
url: /
title: Nico Watine | Houston Web Developer
description: I design, brand, code and maintain mobile-optimized websites
og-title: Nico Watine | Houston Web Developer
og-description: My name is Nico. I design, brand, code and maintain mobile-optimized websites for projects of all sizes.
og-image:
body-id: index
body-class: index
---
<div class="row flex heightWrapper">
	<div itemscope itemtype="http://schema.org/Person" class="pageContent columns flex">
		<h1 itemprop="name" class="margin-0"><strong>Nico Watine</strong></h1>
		<p itemprop="description" class="h4 summary">I design, brand, code and maintain mobile-optimized websites for projects of all sizes. I’m located in <span itemprop="addressLocality">Houston</span>, <span itemprop="addressRegion">TX</span>, and am available for hire anywhere.</p>
		<br>
		<h3 class="margin-0"><strong>Portfolio</strong></h3>
		<a itemprop="url" href="https://studioncreations.com/" class="h4 colophonLink inline-block">https://studioncreations.com</a>
	</div>
</div>
<div class="gradient-bg"></div>
<div class="bg-img"></div>